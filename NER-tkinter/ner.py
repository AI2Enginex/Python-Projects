...
class GenerateData(LoadFile):
    ...
    def process_data(self,column_name):
       ...
       return [re.sub('[^a-zA-Z]',' ' , message.lower())  for message in self.file[column_name]]
...
class NameEntityRecognition:
    ...
    def __init__(self,file_name,model):
        ...
        self.phrase = PhraseAnalysis(file_name,model)
...
class WordNER:
    ...
    def generate_list(self,feature_name,word):
        ...
        return [re.sub('[^a-zA-Z]',' ' , message.lower())  for message in df[feature_name]]
...
class GetNameEntity(WordNER):
    ...
    def entityfile(self,column_name,word,csv_file_name):
        ...
        df = pd.DataFrame.from_dict(self.namentity(column_name,word),orient='index',columns=['text'])
        df.to_csv(csv_file_name)
...
class GUI:
    ...
    def __init__(self):
        ...
        tk.Label(self.top, text="Model name",
             font=("Helvetica", 14)).place(x=80, y=125)
        ...
        self.canvas.create_window(280,140 , window=self.e4)
    ...
    def run_app(self):
        ...
        try:
            name = NameEntityRecognition(val1,val4)
    ...
    def buttons(self):
        ...
        tk.Button(self.top, text='Create CSV', command=self.run_app).place(x=120,y=160)
        tk.Button(self.top, text='filter a word', command=self.filter_by_word).place(x=220,y=160)
